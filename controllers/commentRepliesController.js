exports.getReplies = (req, res) => {
    ...
    async function getReplies() {
        try {
            ...
            var db3 = new sqlite3.Database('C:\\Users\\Susana Rijo\\Source\\repos\\Smf\\YouBlog\\Server\\Digital-Marketing-Guru.db', (err) => {
                if (err) {
                    console.error(err.message);
                    return;
                }
                let repliesSql = 'SELECT * FROM Comments WHERE BlogPostSlug = "' + slug + '" AND ParentCommentId = ' + commentId;
                db3.all(repliesSql, [], (err, replies) => {
                    if (err) {
                        console.error(err.message);
                    ...
                    replies.forEach((reply) => {
                        htmlReplies += '<h6 class="card-title mbr-fonts-style display-7 mt-1"><strong>Reply from ' + reply.Author + ':</strong></h6>' +
                            ...
                            '   <div class="modal-dialog modal-sm" style="height:auto" role="document">' +
                            ...
                            '                   <input Type="hidden" id="BlogPostId' + reply.CommentId + '" name="BlogPostId' + reply.CommentId + '" value="' + reply.BlogPostId + '" />' +
                            '               </form > ' +
                            '           </div>' +
